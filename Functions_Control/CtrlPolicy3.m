function [fk,fkc] = CtrlPolicy3(q,err,K)
...
% fc_vi2di   = fkc_a_p + fkc_b_n + fkc_ab_p;
% fc_vii2dii = fkc_a_n + fkc_b_p + fkc_ab_p;
...
fc_vi2di   = fkc_a_p;
fc_vii2dii = fkc_b_p;
fc_di2dii  = fkc_theta_p + fkc_a_n + fkc_b_n;
fc_vi2vii  = fkc_theta_n + fkc_a_n + fkc_b_n;
fc_di2vii  = fkc_alpha_p;
fc_vi2dii  = fkc_alpha_n;

% fc_vi2di   = zeros(1,n_sec);
% fc_vii2dii = zeros(1,n_sec);
% fc_di2dii  = zeros(1,n_sec);
% fc_vi2vii  = zeros(1,n_sec);
% fc_di2vii  = zeros(1,n_sec);
% fc_vi2dii  = zeros(1,n_sec);
% 
% fc_vi2di(1:end-1)   = fkc_a_p(1:end-1);
% fc_vii2dii(1:end-1) = fkc_b_p(1:end-1);
% fc_di2dii(1:end-1)  = fkc_theta_p(1:end-1) + fkc_ab_n(1:end-1);
% fc_vi2vii(1:end-1)  = fkc_theta_n(1:end-1) + fkc_ab_n(1:end-1);
% fc_di2vii(1:end-1)  = fkc_alpha_p(1:end-1);
% fc_vi2dii(1:end-1)  = fkc_alpha_n(1:end-1);

% fc_vi2di(end)   = fkc_a_p(end) + fkc_b_n(end) + fkc_ab_p(end);
% fc_vii2dii(end) = fkc_a_n(end) + fkc_b_p(end) + fkc_ab_p(end);
% fc_di2dii(end)  = fkc_theta_p(end) + fkc_ab_n(end);
% fc_vi2vii(end)  = fkc_theta_n(end) + fkc_ab_n(end);
% fc_di2vii(end)  = fkc_alpha_p(end);
% fc_vi2dii(end)  = fkc_alpha_n(end);
...
fki_vi2di =  Fi_vi2di_npair_fcn([fc_vi2di; 0],q);
fki_vii2dii = Fi_vi2di_npair_fcn([0; fc_vii2dii],q);
fki_di2dii = Fi_di2dii_fcn(fc_di2dii,q);
fki_vi2vii = Fi_vi2vii_fcn(fc_vi2vii,q);
fki_di2vii = Fi_di2vii_fcn(fc_di2vii,q);
fki_vi2dii = Fi_vi2dii_fcn(fc_vi2dii,q);
% fc_di2dii  = fkc_theta_p + fkc_a_n + fkc_b_n;
% fc_vi2vii  = fkc_theta_n + fkc_a_n + fkc_b_n;
% fc_di2vii  = fkc_alpha_p;
% fc_vi2dii  = fkc_alpha_n;
...
% fk = fki_alpha + fki_theta + fki_a + 0*fki_b + fki_ab;
fk = fki_vi2di +fki_vii2dii + fki_di2dii + fki_vi2vii + fki_di2vii + fki_vi2dii;
